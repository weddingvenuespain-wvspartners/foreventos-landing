...
const APP_URL = 'https://app.foreventos.com'
...
const TRANSLATIONS = {
  en: {
    ...
    problems: {
      ...
      items: [
        ...
        { n: '04', title: 'A PDF doesn\'t sell your venue — an experience does', desc: 'Your spaces deserve more than an email attachment. Interactive digital proposals convert up to 3x more than a static PDF.' },
      ],
    },
    how: {
    ...
    pricing: {
      ...
      cta: 'Start free',
      plans: [
        { name: 'Basic', desc: 'Entry plan for venues', monthly: 35, annual: 350, features: ['Venue listing on directory', 'Lead management', 'Availability calendar', 'Basic analytics'] },
        { name: 'Premium', desc: 'For professional venues', monthly: 55, annual: 550, features: ['Venue listing on directory', 'Lead management', 'Availability calendar', 'Basic analytics', 'Sales pipeline', 'Digital proposals', 'Rates & pricing configuration', 'Advanced analytics', 'Priority support'] },
      ],
    },
    faq: {
      eyebrow: 'FAQ',
    ...
    footer: {
      ...
      copy: '© 2026 ForEventos. All rights reserved.',
    },
  },
  ...
  es: {
    ...
    pricing: {
      ...
      cta: 'Empieza gratis',
      plans: [
        { name: 'Básico', desc: 'Plan de entrada para venues', monthly: 35, annual: 350, features: ['Ficha en el directorio', 'Gestión de leads', 'Calendario de disponibilidad', 'Estadísticas básicas'] },
        { name: 'Premium', desc: 'Para venues profesionales', monthly: 55, annual: 550, features: ['Ficha en el directorio', 'Gestión de leads', 'Calendario de disponibilidad', 'Estadísticas básicas', 'Pipeline de ventas', 'Propuestas digitales', 'Tarifas y configuración', 'Estadísticas avanzadas', 'Soporte prioritario'] },
      ],
    },
    faq: {
      eyebrow: 'FAQ',
    ...
    footer: {
      ...
      copy: '© 2026 ForEventos. Todos los derechos reservados.',
    },
  },
} as const
...
const IMGS = {
  leads: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=900&q=85&auto=format&fit=crop',
  prop:  'https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=900&q=85&auto=format&fit=crop',
  cal:   'https://images.unsplash.com/photo-1506784983877-45594efa4cbe?w=900&q=85&auto=format&fit=crop',
} as const
    ...
    <div style={{ fontFamily: SANS, color: C.dark, background: C.white, overflowX: 'hidden' }}>
      ...
      <header style={{
        ...
        <a href="/" style={{ textDecoration: 'none', lineHeight: 0 }}>
          <img src={LOGO_URL} alt="ForEventos" style={{ height: 32, display: 'block' }} />
        </a>
        ...
        {!isMobile && (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <a href={`${APP_URL}/signup`} style={{
              ...
              transition: 'background 0.2s', minWidth: 140, textAlign: 'center',
            }}>{t.nav.register}</a>
          </div>
        )}
      ...
      {isMobile && mobileMenu && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: C.darkDeep, display: 'flex', flexDirection: 'column', padding: '88px 32px 32px' }}>
          ...
          <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <a href={`${APP_URL}/login`} style={{ textDecoration: 'none', fontFamily: SANS, fontSize: 15, fontWeight: 500, color: C.white, textAlign: 'center', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, padding: '14px 0' }}>{t.nav.login}</a>
            <a href={`${APP_URL}/signup`} style={{ textDecoration: 'none', fontFamily: SANS, fontSize: 15, fontWeight: 600, color: C.white, textAlign: 'center', background: C.accent, borderRadius: 8, padding: '14px 0' }}>{t.nav.register}</a>
          </div>
        </div>
      )}

      {/* ─── HERO ─── */}
      <section style={{ minHeight: '100vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '96px 24px 130px' : '68px 60px 0' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 1, background: isMobile ? 'linear-gradient(to bottom, rgba(44,38,20,0.80) 0%, rgba(44,38,20,0.88) 100%)' : 'linear-gradient(105deg, rgba(44,38,20,0.90) 0%, rgba(44,38,20,0.75) 55%, rgba(44,38,20,0.40) 100%)' }} />
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 1100, width: '100%', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 40 : 0 }}>
          <div style={{ flex: isMobile ? undefined : '0 0 58%', maxWidth: isMobile ? '100%' : 640 }}>
            ...
            <p style={{ fontFamily: SANS, fontSize: isMobile ? 15 : 18, color: 'rgba(255,255,255,0.52)', maxWidth: 480, lineHeight: 1.75, marginTop: 20, marginBottom: 0 }}>{t.hero.sub}</p>
            <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 10, marginTop: 32, alignItems: isMobile ? 'stretch' : 'center' }}>
              <a href={`${APP_URL}/signup`} style={{ textDecoration: 'none', fontFamily: SANS, fontSize: 15, fontWeight: 600, color: C.white, background: C.accent, padding: '14px 32px', borderRadius: 6, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>{t.hero.cta} <ChevronRight size={15} /></a>
              <button onClick={() => scrollTo('funcionalidades')} style={{ fontFamily: SANS, fontSize: 15, fontWeight: 500, color: 'rgba(255,255,255,0.70)', background: 'transparent', padding: '14px 28px', borderRadius: 6, cursor: 'pointer', border: '1px solid rgba(255,255,255,0.18)', transition: 'all 0.2s' }}>{t.hero.ctaSub}</button>
            </div>
          </div>
          {!isMobile && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 14 }}>
              {t.hero.stats.map(s => (
                <div key={s.label} style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '14px 22px', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', gap: 14, minWidth: 210 }}>
      ...
      <section style={{ background: C.darkDeep, borderBottom: '1px solid rgba(255,255,255,0.06)', padding: isMobile ? '20px 24px' : '0 60px' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'center', justifyContent: 'center' }}>
          {[
            { label: lang === 'es' ? 'Venues de bodas' : 'Wedding venues', active: true, soon: false },
            { label: 'Wedding Planners', active: false, soon: true },
            { label: 'Catering', active: false, soon: true },
          ].map((item, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: isMobile ? '14px 0' : '18px 32px', borderLeft: !isMobile && i > 0 ? '1px solid rgba(255,255,255,0.08)' : 'none', borderTop: isMobile && i > 0 ? '1px solid rgba(255,255,255,0.06)' : 'none', width: isMobile ? '100%' : 'auto' }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: item.active ? C.accent : 'rgba(255,255,255,0.18)', flexShrink: 0 }} />
              <span style={{ fontFamily: SANS, fontSize: 13, fontWeight: item.active ? 600 : 400, color: item.active ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.35)' }}>{item.label}</span>
              {item.soon && <span style={{ fontFamily: SANS, fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: C.accent, background: 'rgba(121,111,78,0.12)', border: '1px solid rgba(121,111,78,0.25)', borderRadius: 4, padding: '2px 7px' }}>{lang === 'es' ? 'Próximamente' : 'Coming soon'}</span>}
            </div>
          ))}
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {t.problems.items.map((item, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 12 : 40, padding: isMobile ? '28px 0' : '36px 0', borderTop: '1px solid rgba(255,255,255,0.07)', alignItems: isMobile ? 'flex-start' : 'center' }}>
                ...
                {!isMobile && <div style={{ flexShrink: 0 }}><div style={{ width: 36, height: 36, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ArrowRight size={14} color="rgba(255,255,255,0.3)" /></div></div>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 48 : 80, alignItems: isMobile ? 'flex-start' : 'center' }}>
            <div style={{ flex: '0 0 auto', maxWidth: isMobile ? '100%' : 340 }}>
              ...
              <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 36 : 50, fontWeight: 700, color: C.dark, margin: '0 0 20px', lineHeight: 1.1 }}>{t.how.title}<br /><em>{t.how.titleItalic}</em></h2>
              <p style={{ fontFamily: SANS, fontSize: 15, color: C.muted, lineHeight: 1.8, margin: '0 0 32px' }}>{t.how.sub}</p>
              <a href={`${APP_URL}/signup`} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, textDecoration: 'none', fontFamily: SANS, fontSize: 14, fontWeight: 600, color: C.white, background: C.dark, padding: '12px 24px', borderRadius: 6 }}>{t.how.cta} <ArrowRight size={14} /></a>
            </div>
      ...
      <section id="funcionalidades" style={{ background: C.white, padding: isMobile ? '72px 24px' : '110px 60px' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: isMobile ? 52 : 72 }}>
            ...
            <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 36 : 52, fontWeight: 700, color: C.dark, margin: 0, lineHeight: 1.1 }}>{t.features.title}<br /><em>{t.features.titleItalic}</em></h2>
          </div>
          {t.features.items.map((f, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: isMobile ? 'column' : (i % 2 === 1 ? 'row-reverse' : 'row'), gap: isMobile ? 32 : 72, padding: isMobile ? '40px 0' : '64px 0', borderBottom: i < 2 ? `1px solid ${C.border}` : 'none', alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {f.bullets.map(b => (
                    <div key={b} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 18, height: 18, borderRadius: '50%', background: C.accentLight, border: `1px solid ${C.accentBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Check size={10} color={C.accent} strokeWidth={2.5} /></div>
                      <span style={{ fontFamily: SANS, fontSize: 14, color: C.dark, fontWeight: 500 }}>{b}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ flex: 1, height: isMobile ? 220 : 380, borderRadius: isMobile ? 12 : 16, backgroundImage: `url(${IMGS[f.img as keyof typeof IMGS]})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundColor: C.bg, boxShadow: isMobile ? '0 8px 32px rgba(69,61,35,0.12)' : '0 32px 80px rgba(69,61,35,0.18)', border: `1px solid ${C.border}` }} />
            </div>
          ))}
      ...
      <section id="planes" style={{ background: C.white, padding: isMobile ? '72px 24px' : '110px 60px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 36 : 50, fontWeight: 700, color: C.dark, margin: '0 0 12px', lineHeight: 1.1 }}>{t.pricing.title}<br /><em>{t.pricing.titleItalic}</em></h2>
            <p style={{ fontFamily: SANS, fontSize: 15, color: C.muted, margin: '0 0 20px' }}>{t.pricing.sub}</p>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.accentLight, border: `1px solid ${C.accentBorder}`, borderRadius: 20, padding: '6px 16px', fontSize: 13, color: C.accent, fontFamily: SANS, fontWeight: 500 }}><Check size={13} /> {t.pricing.trial}</div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2,1fr)', gap: 24 }}>
            {t.pricing.plans.map((plan, idx) => {
              ...
              return (
                <div key={plan.name} style={{ borderRadius: 16, padding: isMobile ? '28px 22px' : '36px 32px', border: isPremium ? `2px solid ${C.accent}` : `1px solid ${C.border}`, background: isPremium ? C.bg : C.white, position: 'relative', boxShadow: isPremium ? '0 12px 40px rgba(121,111,78,0.12)' : undefined }}>
                  ...
                  <div style={{ fontFamily: SERIF, fontSize: 24, fontWeight: 700, color: C.dark, marginTop: isPremium ? 10 : 0 }}>{plan.name}</div>
                  <div style={{ fontFamily: SANS, fontSize: 13, color: C.muted, marginTop: 4, lineHeight: 1.5 }}>{plan.desc}</div>
                  <div style={{ margin: '20px 0', paddingBottom: 20, borderBottom: `1px solid ${C.border}` }}>
                    <span style={{ fontFamily: SERIF, fontSize: 44, fontWeight: 700, color: C.dark, letterSpacing: '-0.02em' }}>{price}€</span>
                    ...
                    {billingAnnual && <div style={{ fontFamily: SANS, fontSize: 12, color: C.muted, marginTop: 4 }}>{Math.round(price / 12)}€{t.pricing.billedAnnually}</div>}
                  </div>
                  <div>
                    {plan.features.map(f => (
                      <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '5px 0' }}>
                        ...
                        <span style={{ fontFamily: SANS, fontSize: 13, color: C.dark }}>{f}</span>
                      </div>
                    ))}
                  </div>
                  <a href={`${APP_URL}/signup`} style={{ display: 'block', textAlign: 'center', textDecoration: 'none', fontFamily: SANS, fontSize: 14, fontWeight: 600, color: C.white, background: isPremium ? C.accent : C.dark, padding: '14px 0', borderRadius: 8, marginTop: 28, transition: 'opacity 0.2s' }}>{t.pricing.cta}</a>
                </div>
              )
            })}
          </div>
      ...
      <section style={{ background: C.bg, padding: isMobile ? '72px 24px' : '110px 60px' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          ...
          <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 36 : 50, fontWeight: 700, color: C.dark, margin: '0 0 44px', lineHeight: 1.1 }}>{t.faq.title}<br /><em>{t.faq.titleItalic}</em></h2>
          {t.faq.items.map((item, i) => (
            <div key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
              ...
              {openFaq === i && <div style={{ paddingBottom: 22 }}><p style={{ fontFamily: SANS, fontSize: 14, color: C.muted, lineHeight: 1.8, margin: 0 }}>{item.a}</p></div>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: C.dark, padding: isMobile ? '80px 24px 140px' : '110px 60px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <img src={LOGO_URL} alt="ForEventos" style={{ height: 40, display: 'block', margin: '0 auto 36px', opacity: 0.80 }} />
          <h2 style={{ fontFamily: SERIF, fontSize: isMobile ? 38 : 60, fontWeight: 700, color: C.white, margin: '0 0 20px', lineHeight: 1.05 }}>{t.cta.title}<br /><em style={{ color: '#D4C49A' }}>{t.cta.titleItalic}</em></h2>
          ...
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 12, alignItems: 'center', justifyContent: 'center' }}>
            <a href={`${APP_URL}/signup`} style={{ textDecoration: 'none', fontFamily: SANS, fontSize: 16, fontWeight: 600, color: C.white, background: C.accent, padding: '16px 40px', borderRadius: 6, display: 'inline-flex', alignItems: 'center', gap: 8 }}>{t.cta.btn} <ArrowUpRight size={16} /></a>
            <a href={`${APP_URL}/login`} style={{ textDecoration: 'none', fontFamily: SANS, fontSize: 15, fontWeight: 500, color: 'rgba(255,255,255,0.55)', padding: '16px 28px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.12)' }}>{t.cta.btnSub}</a>
          </div>
      ...
      <footer id="contacto" style={{ background: C.darkDeep, padding: isMobile ? '40px 24px 100px' : '44px 40px', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 24 : 0 }}>
          <div>
            <img src={LOGO_URL} alt="ForEventos" style={{ height: 26, display: 'block', marginBottom: 8, opacity: 0.65 }} />
            <div style={{ fontFamily: SANS, color: 'rgba(255,255,255,0.25)', fontSize: 13 }}>{t.footer.tagline}</div>
          </div>
          <div style={{ display: 'flex', gap: 24 }}>
            <a href="/cookies" style={{ fontFamily: SANS, color: 'rgba(255,255,255,0.30)', fontSize: 13, textDecoration: 'none' }}>{t.footer.cookies}</a>
            <a href="/privacidad" style={{ fontFamily: SANS, color: 'rgba(255,255,255,0.30)', fontSize: 13, textDecoration: 'none' }}>{t.footer.privacy}</a>
          </div>
          <div style={{ fontFamily: SANS, color: 'rgba(255,255,255,0.30)', fontSize: 13 }}>info@foreventos.com</div>
        </div>
      ...
      {isMobile && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 900, background: 'rgba(44,38,20,0.97)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', padding: '12px 20px env(safe-area-inset-bottom, 20px)', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', gap: 10 }}>
          <a href={`${APP_URL}/login`} style={{ flex: 1, textDecoration: 'none', fontFamily: SANS, fontSize: 14, fontWeight: 500, color: 'rgba(255,255,255,0.70)', textAlign: 'center', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '14px 0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{t.nav.login}</a>
          <a href={`${APP_URL}/signup`} style={{ flex: 2, textDecoration: 'none', fontFamily: SANS, fontSize: 14, fontWeight: 600, color: C.white, textAlign: 'center', background: C.accent, borderRadius: 8, padding: '14px 0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{t.nav.register}</a>
        </div>
      )}
    </div>
  )
